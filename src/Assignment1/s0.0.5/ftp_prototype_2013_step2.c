		...
		while (1) 
		{
         ...
         if ( bytes <= 0 ) break;
			...
			if((strncmp(receive_buffer,"STOR",4)==0))
			{
				...
				printf("@alan: the file name=_%s_\n",	fileName);
				printf("@alan:line0\n");
				...
				else
				{
					...
					//while(1)
					//{	
						sprintf(send_buffer,"150 File status okay; about to open data connection zzzzzzzz. \r\n");
						bytes = send(ns, send_buffer, strlen(send_buffer), 0);							
						while(1)
						{
							n = 0;
			...
			if((strncmp(receive_buffer,"RETR",4)==0))
			{
				memset(&fileName, 0, strlen(fileName));
				wy_fileName_collector(receive_buffer,	fileName);
				printf("@alan: the file name=_%s_",	fileName);
				
				if(fopen(fileName,"r")==NULL)
				{
					printf("@alan cant find %s or no such file",	fileName);
...
/**
  * @brief  Return 1 if available, 0 otherwise.
  */
/*
int wy_file_check(char *_fn)
{
	...
	{
		fgets(temp_buffer,78,fin);
		available_Flag = !(strncmp(temp_buffer,	_fn,	strlen(temp_buffer)-2));
	}
	fclose(fin);
