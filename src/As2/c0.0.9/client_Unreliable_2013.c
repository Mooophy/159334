/**
  ...
  * @version V0.0.9
  * @date    03-Oct-2013
  * @brief   fixed the SN_base too small bug by adding an if statement.
  ******************************************************************************
  *	
  */
/* Includes ------------------------------------------------------------------*/
        ...
        */
		...
		if (!feof(fin)) 
		{
			...
			if(SN_next < (SN_base + 4))
			{
				...
				printf("@alan r149: sn_next is %d sn_base is %d\n",SN_next, SN_base);
			...
			*/
		     ...
		     */
			printf("@alan r182: sn_next is %d sn_base is %d\n",SN_next, SN_base);			  
			if(wy_CRC_check(receive_buffer))
			{
				printf("@alan r185: sn_next is %d sn_base is %d\n",SN_next, SN_base);
				if(wy_get_ACK_SN(receive_buffer)>-1)
				{
					SN_base = wy_get_ACK_SN(receive_buffer) + 1;	
				}
				printf("@alan recieve_buffer:%s\n",receive_buffer);
				printf("@alan r187: sn_next is %d sn_base is %d\n",SN_next, SN_base);				
				if(SN_base == SN_next)
				{
					//stop timer
					t.en=0;
			...
			printf("@alan r198: sn_next is %d sn_base is %d\n",SN_next, SN_base);
			Sleep(1000);
		}
		else 
		{
