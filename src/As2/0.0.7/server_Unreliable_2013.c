...
{	
	...
	SOCKET s;
	char send_buffer[BUFFESIZE],receive_buffer[BUFFESIZE],CRC_buffer[10];
	int n,bytes,addrlen;
	...
	while (1) 
	{
	  ...
	  /**
		...
		printf("Alan: SN_expected=%d  SN_checked=%d\n",SN_expected,SN_checked);
		
		//default : send the checked ACK.
		if(SN_checked >= 0)
		{
			//build a packet using SN_checked
			sprintf(send_buffer,"ACKNOW %d", SN_checked);
			sprintf(CRC_buffer, "%d", CRCpolynomial(send_buffer));
			strcat(CRC_buffer," ");
			strcat(CRC_buffer,send_buffer);
			strcpy(send_buffer,CRC_buffer);
			strcat(send_buffer," \r\n");
