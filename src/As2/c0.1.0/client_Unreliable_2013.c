/**
  ...
  * @version V0.1.0
  * @date    03-Oct-2013
  * @brief   1 fixed the SN_base too small bug by adding an if statement.
  *			 2 the new function lost a "\n" before sending.now it's added.	
  ******************************************************************************
  *	
  */
/* Includes ------------------------------------------------------------------*/
        ...
        */
		...
		if (!feof(fin)) 
		{
			...
			if(SN_next < (SN_base + N_of_GBN))
			{
				...
				printf("@alan new function lenth=%d\n",strlen(sndpkt[SN_next]));
				printf("@alan new function [40]:%d  [41]:%d  [42]:%d\n",sndpkt[SN_next][40],sndpkt[SN_next][41],sndpkt[SN_next][42]);
...
/*
			...
			counter++;
			strcat(temp_buffer,data_buffer);
			strcpy(data_buffer,temp_buffer);
		
			//---------coding-------
			temp_buffer[strlen(temp_buffer)-1] = 0;
			sprintf(CRC_str,"%d ",CRCpolynomial(temp_buffer));
			strcat(CRC_str,data_buffer);
			strcpy(data_buffer, CRC_str);
			//----------------------
			printf("@alan old function lenth=%d\n",strlen(data_buffer));
			printf("@alan old function [40]:%d  [41]:%d  [42]:%d\n",data_buffer[40],data_buffer[41],data_buffer[42]);
			send_unreliably(s,data_buffer, remoteaddr); 
