...
class Timer		//timer to count time delay
{
	protected:
		int en;
		int counting;
...
int main(int argc, char *argv[]) 
{
	...
	char sndpkt[12][BUFFESIZE];	//2d array store data
   ...
   while (1)
	{
		if(t.getState()==1)		//check timer state, 0=unactived
		{
			t.increment();	
		}
		
		memset(data_buffer, 0, sizeof(data_buffer));//clean up the send_buffer before reading the next line
		if(SN_next < (SN_base + N_of_GBN))		//check if sliding window is full to decide if continue to read
		{
		...
		if (!feof(fin)) 	
		{
			if(SN_next < (SN_base + N_of_GBN))			//check if sliding window is full
			{
				build_pkt(data_buffer, SN_next, sndpkt[SN_next]);//construct packect
				send_unreliably(s,sndpkt[SN_next], remoteaddr);//and send
				
				if(SN_base == SN_next)		//check if it's empty
				{
					t.start();		//timer start
				}
				SN_next++;				
			...
			if(t.getCounting() > Timer_Limit)	//if timeout, back-to-n
			{
				t.start();		//timer restart
				for(int i = SN_base;i < SN_next;i++ )
				{
					send_unreliably(s,sndpkt[i], remoteaddr);		//keep sending packects
					Sleep(1);		//delay 1 ms
				}
			}
			Sleep(1000);
			...
			if(CRC_check(receive_buffer))		//if match
			{
				if(get_ACK_SN(receive_buffer)>=0)	//test if get_ACK works
				{
					SN_base = get_ACK_SN(receive_buffer) + 1;		//sliding window moving forward
				}
				
				if(SN_base == SN_next)		//if reach sliding window's end
				{
					t.stop();	//time stop and clear
					t.clear();
				}
				else
				{
...
void build_pkt(char *_data_buffer, int _next, char *_sndpkt)	//construct a packet
{
...
int get_ACK_SN(char *_buffer)		//fetch ack and return
{
	char bf[BUFFESIZE];	
...
unsigned int CRC_check(char *_buffer) //check th CRC value match or not
{
	char bf[BUFFESIZE];
