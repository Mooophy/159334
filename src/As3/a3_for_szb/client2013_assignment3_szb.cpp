...
void	RSA_encryptor(Key *_key, char *_buffer);
...
int main(int argc, char *argv[]) 
{
   ...
   while (1) 
	...
	key_builder(&key, receive_buffer);		//fetch key from received string into &key
	printf("received public key: %d, %d\n", key.n, key.e);	//display on screen
   ...
   fgets(send_buffer,SEGMENTSIZE,stdin);
	...
	{	
		RSA_encryptor(&key, send_buffer);		//encrypt message in send_buffer via public key, and save to send_buffer
...
void	key_builder(Key *_key, char *_buffer)		//fetch key sent from server
{
	char bf[80];
	...
	for (word = strtok(bf, sep);word;word = strtok(NULL, sep))
	{
		...
		if(wcount == 2) // skip space
		{
			sscanf(word,"%d",&(_key->n));
		}
		
		if(wcount == 3) // skip space
		{			
...
{	
	...
	memset(temp, 0, BUFFESIZE);	//clear mem to 0 with range of BUFFESIZE start from temp
	...
	while(temp[j]!=0)
	{
		...
		Char_temp[2*j +1]		= 0x60	|	(temp[j]& 0x0f);	
		j++;
	}
	
	strcpy(_buffer,Char_temp);	
}
